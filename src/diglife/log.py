...
class Log:
    ...
    def __init__(self, level=logging.INFO,console_level = logging.INFO, log_file_name="app.log"):
    ...
    def get_logger(self):
        ...
        if not logger.handlers:
            ...
            # 5.2 文件处理器细致 (RotatingFileHandler 或 TimedRotatingFileHandler)

            file_handler_debug = RotatingFileHandler(
                self.LOG_FILE_PATH.replace('.log','_debug.log'),
                maxBytes=20 * 1024 * 1024,  # 10 MB
                backupCount=5,
                encoding="utf-8",
            )
            file_handler_debug.setLevel(logging.DEBUG)  # 文件中显示所有指定级别的日志
